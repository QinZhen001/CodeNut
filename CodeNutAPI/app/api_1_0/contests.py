...
@decorators.composed(decorators.route('/contests/<id>/users', methods=['POST']), decorators.json_required,
...
def join_contest(id):
    ...
    user_contest = UserJoinContest(
        user_id=g.current_user.id, contest_id=verify_id(id))
...
@decorators.composed(decorators.route('/contests/<id>/users', methods=['DELETE']),
                     decorators.permission_required(Permission.PROGRAM))
def out_contest(id):
    user_contest = UserJoinContest.query.filter_by(user_id=g.current_user.id, contest_id=verify_id(id),
                                                   is_join=True).first()
    if not user_contest:
...
@decorators.composed(decorators.route('/contests/<id>/users', methods=['PUT']), decorators.json_required,
...
def allow_join_or_out(id):
    ...
    user_contest = UserJoinContest.query.filter_by(
        user_id=verify_id(user_id), contest_id=verify_id(id)).first()
    if not user_contest:
...
def get_all_contests():
    ...
    paginate = Contest.query.order_by(Contest.id.desc()).paginate(
...
def get_contest_info(id):
    ...
    if g.current_user.id:
        user_contest = UserJoinContest.query.filter_by(
            user_id=g.current_user.id, contest_id=verify_id(id)).first()
        if user_contest:
...
@decorators.composed(decorators.route('/contests', methods=['POST']), decorators.json_required,
                     decorators.permission_required(Permission.SPONSOR))
def create_contest():
    keyword = {'title', 'description', 'start_time',
               'end_time', 'auto_approve', 'password'}
...
@decorators.composed(decorators.route('/contests/<id>', methods=['PUT']), decorators.json_required,
                     decorators.permission_required(Permission.SPONSOR))
...
@decorators.composed(decorators.route('/contests/<id>', methods=['DELETE']),
                     decorators.permission_required(Permission.SPONSOR))
