...
int load_c_cpp_syscall_list(const RunConfig *run_config) {
  ...
  int syscalls_whitelist[] = {
      SCMP_SYS(read),       SCMP_SYS(readlink), SCMP_SYS(close),
      SCMP_SYS(mprotect),   SCMP_SYS(write),    SCMP_SYS(arch_prctl),
      SCMP_SYS(writev),     SCMP_SYS(brk),      SCMP_SYS(access),
      SCMP_SYS(exit_group), SCMP_SYS(mmap),     SCMP_SYS(fstat),
      SCMP_SYS(munmap),     SCMP_SYS(sysinfo),  SCMP_SYS(uname),
      SCMP_SYS(lseek),      SCMP_SYS(time),     SCMP_SYS(set_thread_area)};
...
int load_limit(const RunConfig *run_config) {
  ...
  rtmp.rlim_cur =
      (rlim_t)(run_config->limit_list.time_limit * 1.3 / 1000) + 1 <
              rtmp.rlim_max
          ? (rlim_t)(run_config->limit_list.time_limit * 1.3 / 1000) + 1
          : rtmp.rlim_max;
...
void child_process(const RunConfig *run_config) {
  ...
  if (load_limit(run_config) != 0)
    exit(1);
  if (strcmp(run_config->language, "C") == 0 ||
      strcmp(run_config->language, "C++") == 0) {
    if (load_c_cpp_syscall_list(run_config) != 0)
      exit(1);
  } else {
    if (load_general_syscall_list(run_config) != 0)
      exit(1);
  }
...
int runner(const RunConfig *run_config, RunResult *run_result) {
  ...
  if (child_pid < 0) {
    ...
    if (pthread_create(&monitor_tid, NULL, timeout_killer,
                       (void *)(&killer_args)) != 0) {
    ...
    if (WIFSIGNALED(status) != 0)
      run_result->run_signal = WTERMSIG(status);
    run_result->return_code = WEXITSTATUS(status);
    run_result->time_used = (int)(end.tv_sec * 1000 + end.tv_usec / 1000 -
